...
class App:
    ...
    def _build_gui(self) -> None:
        ...
        btn_select = tk.Button(
            frame_top, text="Select File", command=self.select_file
        )
        ...
        self.lbl_password = tk.Label(
            frame_mid, text="Current Password: (generating...)", anchor="w"
        )
        self.lbl_password.pack(fill=tk.X)

        self.lbl_countdown = tk.Label(
            frame_mid, text="Next rotation in: -- s", anchor="w"
        )
        ...
        btn_decrypt = tk.Button(
            frame_buttons, text="Decrypt File", command=self.decrypt_file_dialog
        )
    ...
    def _on_new_password(self, password: str) -> None:
        ...
        if not self.email_profiles:
            msg = "[Email] Email configuration not found. Skipping email."
            self.log(msg)
        else:
            self.log(
                "[Email] Sending new password to configured "
                "recipients via all profiles..."
            )
    ...
    def encrypt_selected(self) -> None:
        if self.selected_file is None:
            messagebox.showwarning(
                "No file selected", "Please select a file to encrypt."
            )
            return
        ...
        if not password:
            messagebox.showerror(
                "Password not ready",
                "Encryption password is not ready yet. "
        ...
        try:
            ...
            self.log(
                f"[Encrypt] Encrypted '{input_path}' -> '{output_path}' "
                "using current password."
            )
            messagebox.showinfo(
                "Success", f"File encrypted to:\n{output_path}"
            )
    ...
    def decrypt_file_dialog(self) -> None:
        ...
        password = simpledialog.askstring(
            "Decryption Password",
            "Enter the password that was active when the file was encrypted "
            "(check your email):",
            show="*",
        )
        if password is None:
        ...
        try:
            ...
            self.log(
                f"[Decrypt] Decrypted '{input_path}' -> "
                f"'{output_path}'."
            )
            messagebox.showinfo(
                 "Success",
