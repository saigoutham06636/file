...
from email_alerts import (
    ...
    send_file_encryption_email_with_override,
    EmailConfig,
    send_file_encryption_email_async,
)
import json
...
def load_rotation_interval(default: int = 30) -> int:
    ...
    try:
        with open(CONFIG_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        rotation_cfg = data.get("key_rotation", {})
        return int(rotation_cfg.get("interval_seconds", default))
    except Exception:
        return default
...
app.secret_key = os.environ.get(
    "FLASK_SECRET_KEY", "dev-secret-key")  # for flash messages / demo
...
def on_new_password(password: str) -> None:
    ...
    has_email = (
        ...
        and current_sender_password
        and current_recipient_email
    )
    if not has_email:
        return

    # Use first profile as SMTP template (server/port/TLS), override sender
        ...
        with _rotation_lock:
            if _rotation_thread is None or not _rotation_thread.is_alive():
                ...
                _rotation_thread = threading.Thread(
                    target=_rotation_loop, daemon=True)
...
def index():
    return render_template(
        "index.html",
        current_password=key_manager.current_password,
...
def set_email_settings():
    ...
    if not sender or not app_password or not receiver:
        flash(
            "Sender email, app password, and receiver email are all required.",
            "error",
        )
    else:
        ...
        flash(
            f"Email settings updated. Keys sent from {sender} to {receiver}.",
            "success",
        )
...
def encrypt_route():
    ...
    if not key_manager.current_password:
        flash(
            "Encryption password not ready. Please try again in a moment.",
            "error",
        )
        return redirect(url_for("index"))
    ...
    try:
        encrypt_file(str(upload_path), str(enc_path),
                     key_manager.current_password)
    except Exception as e:
    ...
    if email_profiles and has_email:
        send_file_encryption_email_with_override(
            template=email_profiles[0],
            sender_email=current_sender_email,
            sender_password=current_sender_password,
        ...
        send_file_encryption_email_async(
            fallback_config, enc_filename, key_manager.current_password
        )

    if current_recipient_email:
        flash(
            f"File encrypted successfully as {enc_filename}. "
            f"Email sent to {current_recipient_email}.",
            "success",
        )
    else:
...
def decrypt_route():
    ...
    # Build decrypted output filename
    ...
    try:
        ...
        flash(
            "Decryption failed: wrong password or older rotation. "
            "Use the key active at the time of encryption.",
            "error",
        )
        return redirect(url_for("index"))
